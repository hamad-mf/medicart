...
class CartScreenController extends StateNotifier<CartScreenState> {
  ...
  Future<void> incrementItemCount(String cartId, String cartItemId,
      num currentCount, num pricePerItem) async {
    try {
  ...
  Future<void> decrementItemCount(String cartId, String cartItemId,
      num currentCount, num pricePerItem) async {
    try {
      if (currentCount > 1) {
        final newCount = currentCount - 1;
  ...
  Future<void> deleteCartItemAndUpdateTotal(
      String userId, String cartItemId) async {
